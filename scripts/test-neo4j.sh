#!/usr/bin/env bash
# Full integration test: Neo4j via docker-compose + CGC indexing + MCP tools
set -euo pipefail
...
echo ""
echo "=== 3. CLI smoke test: cgc index ==="
uv run cgc index tests/fixtures/sample_projects/sample_project_yaml_infra --force

echo ""
echo "=== 4. CLI smoke test: cgc list ==="
uv run cgc list
