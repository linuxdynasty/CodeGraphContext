"""Integration tests for the full CGC cloud-to-code linking flow.
...
def indexed_yaml(db, graph_builder):
    ...
    asyncio.run(
        graph_builder.build_graph_from_path_async(
            YAML_FIXTURE, is_dependency=False
        )
    )
    return db
...
class TestInfraNodesCreated:
    ...
    def test_k8s_resources_indexed(self, indexed_yaml):
        ...
        with driver.session() as s:
            result = s.run(
                "MATCH (k:K8sResource) RETURN count(k) as cnt"
            ).single()
    ...
    def test_crossplane_xrd_and_claim(self, indexed_yaml):
        ...
        with driver.session() as s:
            xrds = s.run(
                "MATCH (x:CrossplaneXRD) RETURN count(x) as cnt"
            ).single()
            claims = s.run(
                "MATCH (c:CrossplaneClaim) RETURN count(c) as cnt"
            ).single()
            assert xrds["cnt"] >= 1
            assert claims["cnt"] >= 1
    ...
    def test_helm_chart_and_values(self, indexed_yaml):
        ...
        with driver.session() as s:
            charts = s.run(
                "MATCH (h:HelmChart) RETURN count(h) as cnt"
            ).single()
            values = s.run(
                "MATCH (h:HelmValues) RETURN count(h) as cnt"
            ).single()
            assert charts["cnt"] >= 1
            assert values["cnt"] >= 1
    ...
    def test_kustomize_overlay(self, indexed_yaml):
        ...
        with driver.session() as s:
            result = s.run(
                "MATCH (k:KustomizeOverlay) RETURN count(k) as cnt"
            ).single()
            assert result["cnt"] >= 1
...
class TestEcosystemHandlers:
    ...
    def test_get_ecosystem_overview_standalone(self, indexed_yaml):
        from codegraphcontext.tools.handlers.ecosystem_handlers import (
    ...
    def test_find_blast_radius_standalone(self, indexed_yaml):
        ...
        result = find_blast_radius(
            indexed_yaml, "sample_project_yaml_infra"
        )
        assert "error" not in result
